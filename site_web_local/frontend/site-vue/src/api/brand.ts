...
export async function searchBrands(query: string) {
  const { data } = await http.get<BrandOut[]>(`/brands/search?q=${encodeURIComponent(query)}`)
  return data
}

export async function getBrandByName(name: string, autoScrape: boolean = false) {
  const { data } = await http.get<BrandOut>(`/brands/name/${encodeURIComponent(name)}?auto_scrape=${autoScrape}`)
  return data
}